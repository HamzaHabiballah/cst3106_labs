...
class Dice {
    constructor(numDice = 5) {
        ...
        this.currentValues = Array(this.numDice).fill(1); // Initialize with default values
    }

    // Roll method to fetch dice values from the server
    async roll() {
        try {
            const response = await fetch('/roll-dices'); // Fetch dice values from the server
            if (!response.ok) {
                throw new Error('Failed to fetch dice values');
            }
            const data = await response.json();
            this.currentValues = data.diceValues; // Update current dice values
            return this.currentValues;
        } catch (error) {
            console.error('Error rolling dice:', error);
            return this.currentValues; // Return existing values on error
        }
    }
...
// YatzyEngine class for score calculation
class YatzyEngine {
...
class YatzyGame {
    constructor() {
        ...
        // Listen for "Roll Dice" button click
        document.getElementById('roll-dice').addEventListener('click', () => this.rollDice());

        // Setup other game events
    ...
    async rollDice() {
        const values = await this.dice.roll(); // Use updated roll method
        this.displayDice(values);
    }

    // Display dice values on the page
    displayDice(values) {
